...
batch_sample_prob_rwkv = jax.jit(vmap(sample_prob_RWKV, (None, None, None, None, 0)), static_argnames=['fixed_params'])
batch_flip_sample = jax.jit(vmap(vmap(vmap(flip_sample, (None, None, 0)), (None, 0, None)), (0, None, None)))
batch_log_amp_RWKV = jax.jit(vmap(log_amp_RWKV, (0, None, None, None, None)), static_argnames = ['fixed_params'])
...
for delta in ((jnp.arange(11))*0.3):
    params = params_init(rnn_type, Nx, Ny, 0, input_size, emb_size, h_size, num_layer, key)
    ny_nx_indices = jnp.array([[[i, j] for j in range(Nx)] for i in range(Ny)])
    ...
    for it in range(0, numsteps):
        ...
        samples, sample_log_amp = batch_sample_prob_rwkv(params, wemb, fixed_params, ny_nx_indices, sample_key)
        ...
        log_all_amp = batch_log_amp_RWKV(sigmas, params, wemb, fixed_params, ny_nx_indices)
        ...
        grads = grad_f(params, fixed_params, wemb, samples, Eloc, T, ny_nx_indices)
        '''
        if it%50 == 0:
            print(len(grads))
            for i in grads[:-1]:
    ...
    test_sample = batch_sample_prob_rwkv(params, fixed_params, ny_nx_indices, key)
