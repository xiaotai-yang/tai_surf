...
parser.add_argument('--Bx', type = float, default=0.0)
...
for u in units:
    ending+='_{0}'.format(u)
Bx_ = [0.5, 1.0, 1.5, 2.0, 3.0, 3.5]
sample_ = [[] for i in range (len(Bx_))]
for i in range (len(Bx_)):
    ...
    for j in range(L):
        for k in range(L):
            spin_correlation[j,k] += np.mean((2*config[:,j,k]-1)*(2*config[:,y,x]-1))-np.mean((2*config[:,j,k]-1))*np.mean(2*config[:,y,x]-1)
    ...
    for j in range(L):
        for k in range(L):
            spin_correlation_matrix[np.abs(j-y), np.abs(k-x)] += spin_correlation[j,k] 
            count[np.abs(j), np.abs(k)] += 1

    count = np.zeros(17)
    spin_correlation_dis = np.zeros(17)
    for j in range (L):
        for k in range (L):
            spin_correlation_dis[np.abs(j-y)+np.abs(k-x)]+= spin_correlation[j,k]
            count[np.abs(j-y)+np.abs(k-x)]+=1

    log_correlation = np.log(np.abs(spin_correlation_dis/count)) 
    np.save("figure/log_connected_correlation_y9x4_J="+str(Bx)+".npy", log_correlation)
    import matplotlib.pyplot as plt
    plt.scatter(np.arange(17), log_correlation)
    plt.ylabel("log_connected_correlation")
    plt.xlabel("distance")
    plt.title("log_correlation_decay_with Bx="+str(Bx), family = "serif")
    plt.savefig("figure/log_connected_correlation_decay_with Bx="+str(Bx)+".png", dpi=150)
    plt.show()
    plt.scatter(np.arange(17), np.exp(log_correlation))
    plt.ylabel("connected_correlation")
    plt.xlabel("distance")
    plt.title("correlation_decay_with Bx="+str(Bx), family = "serif")
    plt.savefig("figure/connected_correlation_decay_with Bx="+str(Bx)+".png", dpi=150)
    plt.show()