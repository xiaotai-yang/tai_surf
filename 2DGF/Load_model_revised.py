...
savename = '_numsamples_'+str(numsamples)+'_magfixed'+str(mag_fixed)+'_mag'+str(magnetization)+'_Jz'+str(Jz)+"_symmetry_"+RNN_symmetry+"_num_gpus"+str(numgpus)
        ...
        with tf.compat.v1.variable_scope(wavefun.scope,reuse=tf.compat.v1.AUTO_REUSE):
            with wavefun.graph.as_default():
                ...
                for i in range (len(position)):
                    ...
                    if choice == "Euclidean":
                        label, real_distance = distance_label(np.array(position[i]), L)
                    else: 
                        label =  cond_label(position[i], L)
                    #length = np.max(label)
                    ...
                    for step_size in range (0, step+1):
                        ...
                        for j in range (num_first_sample//num_units):
                            ...
                            sample_first_prob = np.transpose(sess.run(cond_prob, feed_dict={inputs: sample_first}),(0,2,1))[:,position[i][0],position[i][1]] #transpose
                            ...
                            cond_prob_second = tf.stop_gradient(wavefun.label_sample(num_units, num_second_sample, inputdim = 2, label = label_position, label_value=label_value, position=position[i]))
...
np.save("result/"+str(Jz)+"/tv_"+str(Jz)+"y"+str(args.position_y)+"x"+str(args.position_x)+"_param"+str(args.param), np.array(final_diff))
np.save("result/"+str(Jz)+"/tv_second_var"+str(Jz)+"y"+str(args.position_y)+"x"+str(args.position_x)+"_param"+str(args.param), np.array(final_second_var))
np.save("result/"+str(Jz)+"/tv_first_var"+str(Jz)+"y"+str(args.position_y)+"x"+str(args.position_x)+"_param"+str(args.param), np.array(final_first_var))
