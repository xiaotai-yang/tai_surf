...
ending='_units'
group_character ="A1"
Jz_ = [-1.2, -1.0, -0.8, -0.5, 0.0, 0.5, 0.8, 1.2]
for u in units:
    ending+='_{0}'.format(u)
...
for i in range (len(Jz_)):
    ...
    for i in range(L):
        for j in range(L):
            spin_correlation[i,j] += np.mean((2*config[:,i,j]-1)*(2*config[:,y,x]-1)) - np.mean(2*config[:,i,j]-1)*np.mean(2*config[:,y,x]-1)
    ...
    np.save("figure/connected_log_correlation_y9x4_J="+str(Jz)+".npy", log_correlation)
