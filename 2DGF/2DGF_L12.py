...
parser.add_argument('--mag_fixed', type = str2bool, default=False)
...
parser.add_argument('--XZ', type = str, default='X')
...
Nx=args.L #x dim
Ny=args.L #y dim
num_gpus = get_numavailable_gpus()
#numsamples_estimation = 10**4 number of samples used for the estimation of the energy
...
savename = '_numsamples_'+str(numsamples)+'_magfixed'+str(mag_fixed)+'_mag'+str(magnetization)+"_symmetry_"+RNN_symmetry+"_num_gpus"+str(num_gpus)
...
filename_checkpoint = './Check_Points/Size_'+str(Nx)+'x'+ str(Ny)+'/RNNwavefunction_2DGFRNN_'+XZ+"_"+str(Nx)+'x'+ str(Ny)+ending+savename+'.ckpt'

#If desired to checkpoint from a previous system size
backgroundpath_old = './Check_Points/Size_'+str(Nx-2)+'x'+ str(Ny-2)
filename_oldsyssize = backgroundpath_old+'/RNNwavefunction_2DGFRNN_'+XZ+"_"+str(Nx-2)+'x'+ str(Ny-2)+ending+savename+'.ckpt'
...
with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
    with wf.graph.as_default():
        ...
        if gradient_clip:
            ...
            optimizer=tf.compat.v1.train.GradientDescentOptimizer(learning_rate=learningrate_placeholder)
            gradients, variables = zip(*optimizer.compute_gradients(cost))
            gradients_normalized = []
            for i in range(len(gradients)):
                random_numbers = tf.random.uniform(dtype=tf.float32,shape=tf.shape(gradients[i]))
...
with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
    with wf.graph.as_default():
        ...
        sigmas=np.zeros(((N+1)*numsamples,Nx, Ny), dtype=np.int32)
        H = np.zeros((N+1)*numsamples, dtype=np.float32)
        log_amplitudes = np.zeros((N+1)*numsamples, dtype=np.complex64)
        
        sigmaH = np.zeros(((N+1), Nx, Ny), dtype = np.int32)
        matrixelements=np.zeros((N+1), dtype = np.float32)
        ...
        if dotraining: #If we want to train
            ...
            for it in range(starting_step,numsteps):
                ...
                samples, local_energies = Get_Samples_and_Elocs(Nx, Ny, samples, sigmas, H, log_amplitudes, sigmaH, matrixelements, samples_tensor, inputs, log_amps, sess, RNN_symmetry, XZ)
                ...
                if (it+1)%200==0 :
                    ...
                    np.save(backgroundpath + '/meanEnergy_2DTGCRNN_'+str(Nx)+'x'+ str(Ny) +ending + savename +'.npy', meanEnergy)
                    np.save(backgroundpath + '/varEnergy_2DTGCRNN_'+str(Nx)+'x'+ str(Ny) +ending + savename +'.npy', varEnergy)
...
'''
                if (it+1)%4000 == 0 or it==10:   
                    extra_sample = tf.stop_gradient(wf.sample(int(testing_sample), inputdim=2))
                    extra_samples =  sess.run(extra_sample)
                    np.save("result/sample_L"+str(Nx)+"_"+str(int(testing_sample/10000))+"k_steps"+str(numsteps)+"_"+str(J)+"_"+".npy", extra_samples)

                    sample_cond_prob = sess.run(cond_prob, feed_dict={inputs: extra_samples})
                    np.save("result/sample_cond_prob_L"+str(Nx)+"_"+str(int(testing_sample/10000))+"k_steps"+str(numsteps)+"_"+str(J)+"_"+".npy", sample_cond_prob)
            
            basis = np.reshape(np.array(list(itertools.product([0, 1], repeat=Nx*Ny))), (2**(Nx*Ny), Nx, Ny))
            probs_all_basis = sess.run(log_amps, feed_dict={inputs: basis})
