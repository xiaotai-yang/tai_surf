...
for angle in ((jnp.arange(9)+1)*0.05*jnp.pi):
    ...
    wemb = jnp.eye(emb_size)[:input_size]
    ...
    for it in range(0, numsteps):
        ...
        '''
        t0 = time.time()
        samples, sample_log_amp = batch_sample_prob(params, wemb, fixed_params, ny_nx_indices, sample_key)
        ...
        log_all_amp = batch_log_amp(sigmas, params, wemb, fixed_params, ny_nx_indices)
        ...
        grads = grad_f(params, wemb, fixed_params, samples, Eloc, T, ny_nx_indices)
        #print("Iteration:", it)
        '''
        if it%50 == 0:
            print(len(grads))
