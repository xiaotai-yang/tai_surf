...
savename = '_numsamples_'+str(numsamples)+'_magfixed'+str(mag_fixed)+'_mag'+str(magnetization)+'_Bx'+str(Bx)+"_symmetry_"+RNN_symmetry+"_num_gpus"+str(num_gpus)
    ...
    with wf.graph.as_default():
        ...
        if dotraining: #If we want to train
            ...
            for it in range(starting_step,numsteps):
                ...
                samples, local_energies = Get_Samples_and_Elocs(Bx,J2, Nx, Ny, samples, sigmas, H, log_amplitudes, sigmaH, matrixelements, samples_tensor, inputs, log_amps, sess, RNN_symmetry)
