...
def sample_prob(num_samples, params, fixed_params, key):
    ...
    sample_amp = jnp.sqrt(jnp.prod(probs, axis=(1,2)))*jnp.exp(jnp.sum(phase, axis=(1,2))*1j)  
...
@partial(jax.jit, static_argnames=['fixed_params'])
def log_amp_test(samples, params, fixed_params):
    k = samples.shape[0]
    log_amp = jnp.zeros(k)/2 + jnp.ones(k)*1j
