...
def J1J2_MatrixElements(sigmap,diag_amp,J1,J2):    #sigmap is (num_samples ,input basis)

    #the diagonal part is simply the sum of all Sz-Sz interactions

    diag = jnp.sum(jnp.abs(sigmap[:,:-1]-sigmap[:,1:])*2-1, axis = 1)*J1 #J1 interactions
    diag += jnp.sum(jnp.abs(sigmap[:,:-2]-sigmap[:,2:])*2-1, axis = 1)*J2    #J2 interactions
    ...
    num_avail_basis = jnp.sum(mask, axis=1)
    ...
    temp = sig[ind1[0] , ind1[1]]
    
    sig = sig.at[ind1[0] , ind1[1]].set(sig[ind2[0] , ind2[1]])
    sig = sig.at[ind2[0] , ind2[1]].set(temp)
...
def J1J2_MatrixElements_numpy(sigmap,diag_amp,J1,J2):    #sigmap is (num_samples ,input basis)

    #the diagonal part is simply the sum of all Sz-Sz interactions

    diag = np.sum(np.abs(sigmap[:,:-1]-sigmap[:,1:])*2-1, axis = 1)*J1  #J1 interactions
    diag += np.sum(np.abs(sigmap[:,:-2]-sigmap[:,2:])*2-1, axis = 1)*J2  #J2 interactions
    ...
    num_avail_basis = np.sum(mask, axis=(1))
    ...
    ind1 = nonzero_loc
    ind2 = ind1+np.array([[0],[1]])
    sig = np.repeat(sigmap, num_avail_basis, axis=0)
    temp = sig[ind1[0] , ind1[1]]
    
    sig[ind1[0] , ind1[1]] = sig[ind2[0] , ind2[1]]
    sig[ind2[0] , ind2[1]] = temp
...
@partial(jax.jit, static_argnames=['fixed_params','J1','J2','num_samples'])
def Get_Elocs(J1,J2, num_samples,params, sample_amp, fixed_params):
    Ny, Nx, mag_fixed, magnetization, units = fixed_params      
    matrixelements, sigmas, basis_where = J1J2_MatrixElements(samples, J1, J2, Nx, Ny, params, sample_amp)
    left_basis = (Nx*(Ny-1)+(Nx-1)*Ny)*num_samples-matrixelements.shape[0]
    if left_basis>0:
    ...
    diag_local_E = matrixelements[:num_samples]*amp[:num_samples]
    matrixelements_off_diag, amp_off_diag = matrixelements[num_samples:-left_basis], amp[num_samples:-left_basis]
    basis_where = basis_where.reshape(-1, num_samples+1)
