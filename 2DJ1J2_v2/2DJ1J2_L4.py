...
parser.add_argument('--lrdecaytime', type = float, default=2500)
...
parser.add_argument('--Nannealing', type = int, default=18) #10000
parser.add_argument('--Ntrain', type = int, default=1000)
parser.add_argument('--Nconvergence', type = int, default=30000)
parser.add_argument('--numsamples', type = int, default=100)
parser.add_argument('--testing_sample', type = int, default=1e+5)
...
with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
    with wf.graph.as_default():
        ...
        if RNN_symmetry == "c4vsym":
            ...
            log_amps, cond_prob=wf.log_amplitude_general(inputs,inputdim=2)
            log_amps_tensor, cond_prob_tensor=wf.log_amplitude_general(samples_placeholder,inputdim=2)
...
with tf.compat.v1.variable_scope(wf.scope,reuse=tf.compat.v1.AUTO_REUSE):
    with wf.graph.as_default():
        ...
        sigmas=np.zeros(((8*N+1)*numsamples,Nx, Ny), dtype=np.int32)
        H = np.zeros((8*N+1)*numsamples, dtype=np.float64)
        log_amplitudes = np.zeros((8*N+1)*numsamples, dtype=np.complex64)
        
        sigmaH = np.zeros(((8*N+1), Nx, Ny), dtype = np.int32)
        matrixelements=np.zeros((8*N+1), dtype = np.float64)
        ...
        if dotraining: #If we want to train
            ...
            for it in range(starting_step,numsteps):
                ...
                samples, local_energies = Get_Samples_and_Elocs(J,J2, Nx, Ny, samples, sigmas, H, log_amplitudes, sigmaH, matrixelements, samples_tensor, inputs, log_amps, sess, RNN_symmetry)
                ...
                '''
                if (it+1)%2000 == 0 or it==10:   
                    extra_sample = tf.stop_gradient(wf.sample(int(testing_sample), inputdim=2))
                    extra_samples =  sess.run(extra_sample)
                    np.save("result/sample_L"+str(Nx)+"_"+str(int(testing_sample/10000))+"k_steps"+str(numsteps)+".npy", extra_samples)

                    sample_cond_prob = sess.run(cond_prob, feed_dict={inputs: extra_samples})
                    np.save("result/sample_cond_prob_L"+str(Nx)+"_"+str(int(testing_sample/10000))+"k_steps"+str(numsteps)+".npy", sample_cond_prob)
            '''
                if (it+1)%10000 == 0:
