...
def random_layer_params(ny, nx, m, n, units, key, scale=1e-2):
    w_key, b_key = random.split(key)
    # outkey1, outkey2 = random.split(w_key)
    return (2 * random.uniform(w_key, (ny, nx, m, n)) - 1) / jnp.sqrt(units), (
                2 * random.normal(b_key, (ny, nx, m)) - 1) / jnp.sqrt(units)


# Initialize all layers for a fully-connected neural network with sizes "sizes"
def init_network_params(sizes, ny, nx, units, key):
    keys = random.split(key, len(sizes))
    outkey = keys[0]
    return outkey, [random_layer_params(ny, nx, m, n, units, k) for m, n, k in zip(sizes[1:], sizes[:-1], keys[1:])]
...
def init_gru_params(Nx, Ny, units, input_size, key):
    ...
    key, u_params = init_network_params([2 * (units + input_size), units], Ny, Nx, units, key)
    key, r_params = init_network_params([2 * (units + input_size), units], Ny, Nx, units, key)
    key, s_params = init_network_params([2 * (units + input_size), units], Ny, Nx, units, key)
    key, c1_params = init_network_params([2 * units, units], Ny, Nx, units, key)
    key, c2_params = init_network_params([2 * input_size, units], Ny, Nx, units, key)
    key, amp_params = init_network_params([units, input_size], Ny, Nx, units, key)
    key, phase_params = init_network_params([units, input_size], Ny, Nx, units, key)
...
def init_tensor_gru_params(Nx, Ny, units, input_size, key):
    # input is already concantenated
    key, u_params = init_network_params([4 * (units * input_size), units], Ny, Nx, units, key)
    key, r_params = init_network_params([4 * (units * input_size), units], Ny, Nx, units, key)
    key, s_params = init_network_params([4 * (units * input_size), units], Ny, Nx, units, key)
    key, amp_params = init_network_params([units, input_size], Ny, Nx, units, key)
    key, phase_params = init_network_params([units, input_size], Ny, Nx, units, key)
...
def sample_prob(num_samples, params, fixed_params, key):
    ...
    inputs_init_y = jnp.repeat(jnp.expand_dims(jnp.array([1., 0.]), axis=0), Ny, axis=0)
    ...
    def scan_fun_1d(carry_1d, indices):
        ...
        return (
        rnn_states_x_1d, rnn_states_yi_1d, num_spin, num_up, key, inputs_x_1d, inputs_yi_1d, mag_fixed, magnetization,
        num_samples, Ny, Nx, params_1d), (samples_output, new_prob, new_phase, new_state)
    ...
    def scan_fun_2d(carry_2d, indices):  # indices:[[0,0], [0,1], [0,2]...[0,Nx-1]]
        ...
        carry_1d = rnn_states_x, rnn_states_y[:, index], num_spin, num_up, key, inputs_x, inputs_y[:,
                                                                                          index], mag_fixed, magnetization, num_samples, Ny, Nx, params_1d
        ...
        return (
        rnn_states_x, rnn_states_y, num_spin, num_up, key, inputs_x, inputs_y, mag_fixed, magnetization, num_samples,
        Ny, Nx, params_2d), (row_samples, row_prob, row_phase)
    ...
    init = batch_rnn_states_init_x, batch_rnn_states_init_y, 0, jnp.zeros(
        num_samples), key, batch_inputs_init_x, batch_inputs_init_y, mag_fixed, magnetization, num_samples, Ny, Nx, params
...
def log_amp(samples, params, fixed_params):
    ...
    batch_rnn_states_init_x = jnp.repeat(jnp.expand_dims(rnn_states_init_x, axis=0), num_samples, axis=0)
    batch_rnn_states_init_y = jnp.repeat(jnp.expand_dims(rnn_states_init_y, axis=0), num_samples, axis=0)
    batch_inputs_init_x = jnp.repeat(
        jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([1., 0.]), axis=0), Nx, axis=0), axis=0), num_samples,
        axis=0)
    batch_inputs_init_y = jnp.repeat(
        jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([1., 0.]), axis=0), Ny, axis=0), axis=0), num_samples,
        axis=0)
    init = batch_rnn_states_init_x, batch_rnn_states_init_y, 0, jnp.zeros(
