...
def random_layer_params(ny, nx, m, n, units, key, scale=1e-2):
    ...
    return  (2*random.uniform(w_key, (ny, nx, m, n))-1)/jnp.sqrt(units),  (2*random.normal(b_key, (ny, nx, m))-1)/jnp.sqrt(units)
...
def log_amp(samples, params, fixed_params):
    ...
    batch_rnn_states_init_x = jnp.repeat(jnp.expand_dims(rnn_states_init_x, axis=0), num_samples, axis=0)
    batch_rnn_states_init_y = jnp.repeat(jnp.expand_dims(rnn_states_init_y, axis=0), num_samples, axis=0)        
    batch_inputs_init_x = jnp.repeat(jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([1.,0.]), axis=0), Nx, axis=0), axis=0), num_samples, axis = 0) 
    batch_inputs_init_y = jnp.repeat(jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([1.,0.]), axis=0), Ny, axis=0), axis=0), num_samples, axis = 0)  
