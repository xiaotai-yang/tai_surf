...
def sample_prob(num_samples, params, fixed_params, key):
    ...
    def scan_fun(carry, indices):
        ...
        '''
        n = indices
        rnn_states,  rnn_inputs, num_spin, num_up, key, mag_fixed, magnetization, num_samples,N , params = carry
        ...
        rnn_states = new_state
        key, subkey = split(key)
        samples_output =  categorical(subkey, jnp.log(new_prob))#sampling
        rnn_inputs = one_hot_encoding(samples_output) # one_hot_encoding of the sample
        num_up += 1-samples_output 
        num_spin += 1

        return (rnn_states,  rnn_inputs, num_spin, num_up, key, mag_fixed, magnetization, num_samples, N, params), (samples_output, new_prob, new_phase)

    N, mag_fixed, magnetization, units= fixed_params
    n_indices = jnp.array([i for i in range(N)])
    inputs_init = jnp.array([0.,0.])
    ...
    init = batch_rnn_states_init, batch_inputs_init, 0, jnp.zeros(num_samples), key, mag_fixed, magnetization, num_samples, N, params
    ...
    sample_amp = jnp.sum(jnp.log(probs), axis=(1))*0.5+jnp.sum(phase, axis=(1))*1j
...
def log_amp(samples, params, fixed_params):
    ...
    def scan_fun(carry, indices):
        ...
        '''
        n = indices
        rnn_states, rnn_inputs,  num_spin, num_up, mag_fixed, magnetization, samples, num_samples, N,  params = carry
        print(num_up.shape)
        params_point = tuple(p[n] for p in params)
        new_state, new_prob, new_phase = batch_rnn(rnn_inputs, rnn_states, params_point)
        samples_output =  samples[:, n]
        rnn_states = new_state
        #jax.debug.print("num_up:{}", num_up)
        #jax.debug.print("samples_output:{}", samples_output)
        num_up += 1 - samples_output
        rnn_inputs = one_hot_encoding(samples_output) # one_hot_encoding of the sample
        num_spin += 1

        return (rnn_states, rnn_inputs, num_spin, num_up, mag_fixed, magnetization, samples, num_samples, N, params), (samples_output, new_prob, new_phase)
    ...
    batch_rnn_states_init = jnp.repeat(rnn_states_init[None], num_samples, axis = 0)
    batch_inputs_init = jnp.repeat(jnp.array([0.,0.])[None],  num_samples, axis = 0)
    init = batch_rnn_states_init, batch_inputs_init, 0, jnp.zeros(num_samples) , mag_fixed, magnetization, samples, num_samples, N, params
    ...
    probs, phase, samples = jnp.transpose(probs, (1,0,2)),jnp.transpose(phase, (1,0,2)), jnp.transpose(samples, (1,0))
    #jax.debug.print("probs_original:{}", probs)
