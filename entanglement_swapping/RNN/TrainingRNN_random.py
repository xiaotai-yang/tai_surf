...
def MatrixElements(sigmap, sigmaH, matrixelements, periodic = False):
    ...
    sig = np.copy(sigmap)
    sigmaH[num] = sig
    matrixelements[num] = 0
    num += 1
    # random choosing part
    basis = np.random.rand(N)>0.5
    #off-diagonal part: False-> Flip
    for site in range(N):
        if (site == 0):        #ZXIII...II
            sig=np.copy(sigmap)
            melement = -1
            ...
            sigmaH[num] = sig
            matrixelements[num] = melement
            num += 1
            
        elif (site == N-2):    #IIII..IXZZ
            sig=np.copy(sigmap)
            melement = -1
            ...
            sig=np.copy(sigmap)
            melement = -1
            if basis[site-1]==True:
                sig[site-1] = 1-sigmap[site-1]
            else:
                melement *= (-1)**(sig[site-1])
...
def run_(numsteps = 10**5, systemsize = 12,  num_units = 50, num_layers = 1, numsamples = 500, learningrate = 2.5*1e-4, seed = 111):
    ...
    filename='/../Check_Points/RNNwavefunction_N'+str(N)+'_samp'+str(numsamples)+'_lradap'+str(lr)+'_complexGRURNN'+ savename + ending +'_seed'+str(seed)+'.ckpt'
    ...
    with tf.variable_scope(wf.scope,reuse=tf.AUTO_REUSE):
        with wf.graph.as_default():
          ...
          sigmas=np.zeros(((N+1)*numsamples,N), dtype=np.int32) #Array to store all the diagonal and non diagonal sigmas for all the samples (We create it here for memory efficiency as we do not want to allocate it at each training step)
          H = np.zeros((N+1)*numsamples, dtype=np.float32) #Array to store all the diagonal and non diagonal matrix elements for all the samples (We create it here for memory efficiency as we do not want to allocate it at each training step)
          log_amplitudes = np.zeros((N+1)*numsamples, dtype=np.complex64) #Array to store all the diagonal and non diagonal log_probabilities for all the samples (We create it here for memory efficiency as we do not want to allocate it at each training step)

          sigmaH = np.zeros((N+1,N), dtype = np.int32) #Array to store all the diagonal and non diagonal sigmas for each sample sigma
          matrixelements=np.zeros(N+1, dtype = np.float32) #Array to store all the diagonal and non diagonal matrix elements for each sample sigma (the number of matrix elements is bounded by at most 2N)
...
#               print("Magnetization = ", np.mean(2*samples - 1))
              ...
              if it%20==0:
                 np.save('../Check_Points/meanEnergy_N'+str(N)+'_samp'+str(numsamples)+'_lradap'+str(lr)+'_complexGRURNN'+ savename + ending +'_seed'+str(seed)+'.npy',meanEnergy)
                 np.save('../Check_Points/varEnergy_N'+str(N)+'_samp'+str(numsamples)+'_lradap'+str(lr)+'_complexGRURNN'+ savename + ending +'_seed'+str(seed)+'.npy',varEnergy)
...
#               sess.run(optstep,feed_dict={Eloc:local_energies,samp:samples,learningrate_placeholder: lr_decayed})
          ...
          np.save('../Check_Points/seed'+str(seed),basis)
