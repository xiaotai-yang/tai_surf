...
T = T0
emb_key = split(key, 3)
wemb = jnp.tile(random.orthogonal(emb_key[0],  units), (Ny, Nx, 1, 1))
emb_x = random.orthogonal(emb_key[1], input_size)[:Nx]
emb_y = random.orthogonal(emb_key[2], input_size)[:Ny]
fixed_params = Ny, Nx, py, px, num_layer
batch_sample_prob = jax.jit(vmap(partial(sample_prob, wemb = wemb, emb_x = emb_x, emb_y = emb_y), (None, None, None, 0)), static_argnames=['fixed_params'])
batch_log_amp = jax.jit(vmap(partial(log_amp, wemb = wemb, emb_x = emb_x, emb_y = emb_y), (0, None, None, None)), static_argnames=['fixed_params'])
...
for delta in (jnp.array([1.0,])):

    params = init_params(Ny, Nx, units, num_layer, input_size, key)
    ny_nx_indices = jnp.array([[[i, j] for j in range(Nx)] for i in range(Ny)])
    ...
    for it in range(0, numsteps):
        ...
        grads = grad_f(params, fixed_params, samples, Eloc, T, wemb, emb_x, emb_y,  ny_nx_indices)
