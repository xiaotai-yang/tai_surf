...
#                      RNNwavefunction_N64_samp256_lradap0.0005_complexGRURNN_J1J20.24_units_64_zeromag.ckpt.meta
...
def ith_step(wavefun, position, step, L, num_first_sample, num_second_sample, num_unit): 
    ...
    with tf.compat.v1.variable_scope(wavefun.scope,reuse=tf.compat.v1.AUTO_REUSE):
        with wavefun.graph.as_default():
            ...
            for i in range (len(position)):
                ...
                label = np.zeros((L))
                for dis in range (position[i]+1):   #position follows the label in python tradition
                    label[dis] = np.abs(position[i]-dis)
                label[position[i]] = -1
    ...
    # get the coordinate that we need to match
                for step_size in range (0, step+1):
                    ...
                    for j in range (num_first_sample//num_unit):
                        with tf.device(device):
                            sample_first_input = tf.stop_gradient(wavefun.sample(int(num_unit), inputdim=2))
                            sample_first =  sess.run(sample_first_input)
                            sample_first_prob = sess.run(cond_prob, feed_dict={inputs: sample_first})[:,position[i]] 
                            ...
                            cond_prob_second = tf.stop_gradient(wavefun.label_sample(num_unit, num_second_sample, inputdim = 2, label = label_where[0], label_value=label_value, position=position[i]))
    ...
    '''
final_diff, final_first_var, final_second_var= ith_step(wf, list(np.array([4])), 3, 96, 100, 100, 50)

np.save("tv_"+str(J2[0])+"_time_"+str(args.param), np.array(final_diff))
np.save("tv_second_var"+str(J2[0])+"_time_"+str(args.param), np.array(final_second_var))
np.save("tv_first_var"+str(J2[0])+"_time_"+str(args.param), np.array(final_first_var))
