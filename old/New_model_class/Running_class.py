...
for angle in (0., 0.05*jnp.pi, 0.1*jnp.pi, 0.15*jnp.pi, 0.20*jnp.pi, 0.25*jnp.pi, 0.3*jnp.pi, 0.35*jnp.pi, 0.4*jnp.pi, 0.45*jnp.pi, 0.5*jnp.pi):
    ...
    RNNfunction = rnn_function(Nx, Ny, 1, 1, mag_fixed, magnetization, units, key, tensor_gru_rnn_step)
    ### gradient descent and optimization
    grad_f = jax.jit(jax.grad(compute_cost))
    ...
    for it in range(0, numsteps):
        ...
        samples, sample_log_amp = RNNfunction.sample_prob(numsamples, params)
        ...
        log_all_amp = RNNfunction.log_amp(sigmas, params)
        ...
        grads = grad_f(params, samples, sample_log_amp, Eloc, T)
