...
def log_amp(samples, params, fixed_params):
    ...
    batch_inputs_init_x = jnp.repeat(
        jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([0., 0.]), axis=0), Nx, axis=0), axis=0), num_samples,
        axis=0)
    batch_inputs_init_y = jnp.repeat(
        jnp.expand_dims(jnp.repeat(jnp.expand_dims(jnp.array([0., 0.]), axis=0), Ny, axis=0), axis=0), num_samples,
        axis=0)
    init = batch_rnn_states_init_x, batch_rnn_states_init_y, 0, jnp.zeros(
