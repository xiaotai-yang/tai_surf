...
def init_griffin_params(Nx, Ny, units, input_size, key, ln = False, generalized_output=False):
    # input is already concantenated
    key,  state_emb_key_x, state_emb_key_y, u_key, r_key, s_key, out_key = random.split(key, 7)
    state_init_x, state_init_y = jnp.tile(jnp.arange(units)/units, (Nx, 1)), jnp.tile(jnp.arange(units)/units, (Ny, 1))
    Wu, Wr = jnp.zeros((Ny, Nx, input_size**2, units, 2*units)), random.uniform(r_key, (Ny, Nx, input_size**2, units , 2*units), minval=-1e-4, maxval=1e-4)
...
@partial(jax.jit, static_argnames=("ln",))
def griffin_step(input_y, input_x, input_size, state_x, state_y, params, output_params, ln = True):  # local_input is already concantenated

    wln_out, bln_out, Wu, Wr, Wout = params
    input = jnp.outer(lerp(state_x, state_y, mu_s), lerp(input_x, input_y, mu_i)).ravel()
    k = nn.gelu(Wk @ input)
    v = Wv @ input
...
def lerp3(x1, x2, x3, mu1, mu2):
    return x1 + mu1 * (x2 - x1) + mu2 * (x3 - x1)
def lora(x, lam, A, B):
    return lam + B @ nn.tanh(A @ x)
def ddlerp (x1, x2, mu, lam, A, B):
    return x1 + lora(lerp(x1, x2, mu), lam, A, B)*(x2 - x1)

def ddlerp3 (x1, x2, x3, mu1, mu2, lam, A, B):
    return x1 + lora(lerp3(x1, x2, x3, mu1, mu2), lam, A, B)*(x2 - x1)

def time_mixing(x, t_state, decay, bonus, t_mix_k, t_mix_v, t_mix_r, Wk, Wv, Wr, Wk1, Wv1, Wr1, Wk2, Wv2, Wr2, Wout):
    last_x1, last_x2, alpha, beta= t_state
    k = Wk1 @ lerp3(x, last_x1, last_x2, t_mix_k1, t_mix_k2)
    v = Wv1 @ lerp3(x, last_x1, last_x2, t_mix_v1, t_mix_v2)
    r = Wr1 @ lerp3(x, last_x1, last_x2, t_mix_r1, t_mix_r2)
    g = Wg1 @ lerp3(x, last_x1, last_x2, t_mix_g1, t_mix_g2)
    d = lora(ddlerp3(x, last_x1, last_x2, t_mix_d1, t_mix_d2, lam, dA1, dB1), dA2, dB2)
    w = jnp.exp(-jnp.exp(d))
    wkv = s + jnp.diag(u) @ jnp.outer(k, v)
