...
def init_RWKV_params(input_size, emb_size, h_size,  num_layer, out_size, Ny, Nx, key):
    ...
    wemb = random.uniform(emb_key, (Ny, Nx, input_size, emb_size), minval=-1e-4, maxval=1e-4) #input hasn't been connected
    ...
    eps_init_x, eps_init_y = jnp.zeros((Nx, num_layer, h_size)), jnp.zeros((Ny, num_layer, h_size))
    ...
    t_epsout =  0.5*jnp.tile(jnp.eye(h_size), (Ny, Nx, num_layer, 1, 2))
    #t_betaout = random.uniform(key_tbeta_out, (Ny, Nx, num_layer, h_size, 2*h_size), minval = 0.3/h_size, maxval = 0.6/h_size)
    #t_epsout = random.uniform(key_tout, (Ny, Nx, num_layer, h_size, 2*h_size), minval = 0.3/h_size, maxval = 0.6/h_size)
    c_xout = 0.5*jnp.tile(jnp.eye(emb_size),  (Ny, Nx, num_layer, 1, 2))
    ...
    wln_tlastx, bln_tlastx = jnp.ones((Ny, Nx, num_layer, emb_size)), jnp.zeros((Ny, Nx, num_layer, emb_size))
    wln_clastx, bln_clastx = jnp.ones((Ny, Nx, num_layer, emb_size)), jnp.zeros((Ny, Nx, num_layer, emb_size))
    # time mixing params
    ...
    wprob, bprob, wphase, bphase = jnp.zeros((Ny, Nx, out_size, emb_size)), jnp.zeros((Ny, Nx, out_size)), jnp.zeros((Ny, Nx, out_size, emb_size)), jnp.zeros((Ny, Nx, out_size))
    RWKV_cell_params = (wln[0], bln[0], wln[1], bln[1], wln_tlastx, bln_tlastx, wln_clastx, bln_clastx, decay, bonus, t_mix_k, t_mix_v, t_mix_r,
                        t_wk, t_wv, t_wr, t_wout, t_wlast_x, c_mix_k, c_mix_r, c_wk, c_wv, c_wr, c_wlast_x,
                        t_xout, t_alphaout, t_betaout, t_epsout, c_xout)
    ...
    return (wemb, x_init, y_init, t_last_x1_init, t_last_x2_init, t_last_y1s_init, t_last_y1e_init, t_last_y2_init, t_alpha_init_x, t_alpha_init_y,
            t_beta_init_x, t_beta_init_y, eps_init_x, eps_init_y, c_last_x1_init, c_last_x2_init, c_last_y1s_init, c_last_y1e_init, c_last_y2_init,
            wln_in, bln_in, wln_out, bln_out, whead, bhead, wprob, bprob, wphase, bphase, RWKV_cell_params)
@jax.jit
def time_mixing(x, t_state, decay, bonus, t_mix_k, t_mix_v, t_mix_r, t_wlast_x, Wk, Wv, Wr, Wout, t_xout, t_alphaout, t_betaout, t_epsout, wln_tlastx, bln_tlastx):
    last_x, last_alpha, last_beta, eps = t_state
    last_x = layer_norm(t_wlast_x @ last_x, wln_tlastx, bln_tlastx)
    k = Wk @ (x * t_mix_k + last_x * (1 - t_mix_k))
    v = Wv @ (x * t_mix_v + last_x * (1 - t_mix_v))
    r = Wr @ (x * t_mix_r + last_x * (1 - t_mix_r))


    tau = jnp.maximum(bonus+k, eps)

    wkv = (jnp.exp(eps-tau)*last_alpha + jnp.exp(bonus + k - tau) * v) / \
          (jnp.exp(eps-tau)*last_beta + jnp.exp(bonus + k - tau))

    #jax.debug.print("t_x:{}", x)
    #jax.debug.print("t_last_x:{}", last_x)
    jax.debug.print("wkv_numerator_norm:{}", jnp.linalg.norm(jnp.exp(eps-tau)*last_alpha + jnp.exp(bonus + k - tau) * v))
    jax.debug.print("wkv_denominator_norm:{}", jnp.linalg.norm(jnp.exp(eps-tau)*last_beta + jnp.exp(bonus + k - tau)))
    rwkv = nn.sigmoid(r) * wkv
    #jax.debug.print("decay:{}", decay)
    #jax.debug.print("exp(k):{}", jnp.exp(k))
    eps_ = jnp.maximum(-jnp.exp(decay) + eps, k)
    alpha = jnp.exp(-jnp.exp(decay)-eps_+eps) * last_alpha + jnp.exp(k - eps_) * v
    beta = jnp.exp(-jnp.exp(decay)-eps_+eps) * last_beta + jnp.exp(k - eps_)
    eps = eps_
    ...
    x = t_xout @ x
    return Wout @ rwkv, (x, alpha, beta, eps)
@jax.jit
def channel_mixing(x, c_states, c_mix_k, c_mix_r, c_wlast_x, Wk, Wv, Wr, c_xout, wln_clastx, bln_clastx):
    last_x = c_states #change tuple into array
    last_x = layer_norm(c_wlast_x @ last_x, wln_clastx, bln_clastx)
...
def RWKV_step(x, t_states, c_states, num_layer, RWKV_net_params, indices):
    ...
    prob = nn.softmax(wprob[ny, nx] @ x + bprob[ny, nx])
    phase = 2*jnp.pi*nn.soft_sign(wphase[ny, nx] @ x + bphase[ny, nx])
...
def RWKV_cell(carry, i, params, cell_t_states, cell_c_states): # carry = (x, t_states, c_states)
    ...
    x = carry
    (wln_i, bln_i, wln_m, bln_m, wln_tlastx, bln_tlastx, wln_clastx, bln_clastx, decay, bonus, t_mix_k, t_mix_v, t_mix_r, t_wk, t_wv, t_wr, t_wout, t_wlast_x,
     c_mix_k, c_mix_r, c_wk, c_wv, c_wr, c_wlast_x, t_xout, t_alphaout, t_betaout, t_epsout, c_xout) = tuple(p[i] for p in params)
    layer_t_states = tuple(t[i] for t in cell_t_states)

    layer_c_states = cell_c_states[i]

    x_ = layer_norm(x, wln_i, bln_i)
    dx, output_t_states = time_mixing(x_, layer_t_states, decay, bonus, t_mix_k, t_mix_v, t_mix_r, t_wlast_x, t_wk, t_wv,
                                      t_wr, t_wout,  t_xout, t_alphaout, t_betaout, t_epsout, wln_tlastx, bln_tlastx)
    x = x + dx
    x_ = layer_norm(x, wln_m, bln_m)
    dx, output_c_states = channel_mixing(x_, layer_c_states, c_mix_k, c_mix_r, c_wlast_x, c_wk, c_wv, c_wr, c_xout, wln_clastx, bln_clastx)
    x = x + dx
