...
def sample_prob_RWKV(params, fixed_params, ny_nx_indices, key):
    ...
    def scan_fun_1d(carry_1d, indices):
        ...
        (input_x, input_yi, t_last_x1, t_last_x1s, t_last_x1e, t_last_x2, t_last_y1i, t_last_y2i, t_alpha_state_x1, t_beta_state_x1,
         t_alpha_state_yi, t_beta_state_yi, eps_x1, eps_yi, c_last_x1, c_last_x1s, c_last_x1e, c_last_x2, c_last_y1i, c_last_y2i, key) = carry_1d
        ...
        eps_state = jnp.concatenate((eps_yi, eps_x1[nx]), axis=1)
        x, t_states, c_states, prob, phase = RWKV_step(rnn_input, (last_t_state, t_alpha_state, t_beta_state, eps_state),
        ...
        t_last_y1i, t_alpha_state_yi, t_beta_state_yi, eps_yi = t_states
        c_last_y1i = c_states
        ...
        t_beta_state_x1, t_alpha_state_yi, t_beta_state_yi, eps_x1, eps_yi, c_last_x1, c_last_x1s, c_last_x1e, c_last_x2, c_last_y1i, c_last_y2i, key),
        (t_last_y1i, t_alpha_state_yi, t_beta_state_yi, eps_yi, c_last_y1i, block_sample, probs, phase))

    def scan_fun_2d(carry_2d, indices):  # indices:[[0,0], [0,1], [0,2]...[0,Nx-1]]

        (input_x, input_y, t_last_x1, t_last_x2, t_last_y1s, t_last_y1e, t_last_y2, t_alpha_state_x, t_beta_state_x, t_alpha_state_y, t_beta_state_y,
         eps_x, eps_y, c_last_x1, c_last_x2, c_last_y1s, c_last_y1e, c_last_y2, key) = carry_2d
        ...
        index], t_alpha_state_x, t_beta_state_x, t_alpha_state_y[index], t_beta_state_y[index], eps_x, eps_y[index],
        c_last_x1, c_last_x1s, c_last_x1e, c_last_x2, c_last_y1e[index + 1], c_last_y2[index], key)
        _, y = scan(scan_fun_1d, carry_1d, indices)
        '''
        ...
        t_last_x1, t_alpha_state_x1, t_beta_state_x1, eps_x, c_last_x1, row_block_sample, row_prob, row_phase = y
        key = _[-1]
        t_last_x2 = jnp.flip(t_last_x2, 0)
        t_last_x1 = jnp.flip(t_last_x1, 0)
        c_last_x2 = jnp.flip(c_last_x2, 0)
        c_last_x1 = jnp.flip(c_last_x1, 0)
        eps_x = jnp.flip(eps_x, 0)
        t_alpha_state_x1 = jnp.flip(t_alpha_state_x1, 0)
        t_beta_state_x1 = jnp.flip(t_beta_state_x1, 0)
        input_x = wemb[index][jnp.flip(jnp.arange(Nx)), jnp.flip(row_block_sample)]
        row_block_sample = lax.cond(index % 2, lambda x: jnp.flip(x, 0), lambda x: x, row_block_sample)
        row_prob = lax.cond(index % 2, lambda x: jnp.flip(x, 0), lambda x: x, row_prob)
        row_phase = lax.cond(index % 2, lambda x: jnp.flip(x, 0), lambda x: x, row_phase)

        return (input_x, input_y, t_last_x1, t_last_x2, t_last_y1s, t_last_y1e, t_last_y2, t_alpha_state_x1, t_beta_state_x1,
                t_alpha_state_y, t_beta_state_y, eps_x, eps_y, c_last_x1, c_last_x2, c_last_y1s, c_last_y1e, c_last_y2, key), (row_block_sample, row_prob, row_phase)

    # initialization
    wemb = params[0]
    init = (*params[1:19], key)
    RWKV_net_params = params[19:]
    __, (samples, probs, phase) = scan(scan_fun_2d, init, ny_nx_indices)
    samples = vmap(int_to_binary, 0)(samples).reshape(Ny * py, Nx * px)
    log_probs, phase = jnp.sum(jnp.log(probs+1e-27)), jnp.sum(phase)
    samples_log_amp = log_probs / 2 + phase * 1j/(Ny*Nx*py*px)
...
def log_amp_RWKV(samples, params, fixed_params, ny_nx_indices):
    ...
    def scan_fun_1d(carry_1d, indices):
        ...
        (input_x, input_yi, t_last_x1, t_last_x1s, t_last_x1e, t_last_x2, t_last_y1i, t_last_y2i, t_alpha_state_x1, t_beta_state_x1,
         t_alpha_state_yi, t_beta_state_yi,  eps_x1, eps_yi,  c_last_x1, c_last_x1s, c_last_x1e, c_last_x2, c_last_y1i, c_last_y2i) = carry_1d
        ...
        return ((input_x, input_yi, t_last_x1, t_last_x1s, t_last_x1e, t_last_x2, t_last_y1i, t_last_y2i, t_alpha_state_x1,
            t_beta_state_x1, t_alpha_state_yi, t_beta_state_yi, eps_x1, eps_yi, c_last_x1, c_last_x1s, c_last_x1e, c_last_x2, c_last_y1i, c_last_y2i),
            (t_last_y1i, t_alpha_state_yi, t_beta_state_yi, eps_yi,  c_last_y1i, block_sample, probs, phase))

    def scan_fun_2d(carry_2d, indices):  # indices:[[0,0], [0,1], [0,2]...[0,Nx-1]]

        (input_x, input_y, t_last_x1, t_last_x2, t_last_y1s, t_last_y1e, t_last_y2, t_alpha_state_x,
         t_beta_state_x, t_alpha_state_y, t_beta_state_y, eps_x, eps_y, c_last_x1, c_last_x2, c_last_y1s, c_last_y1e, c_last_y2) = carry_2d
        ## The shape of last_y1s, last_y1e are (Ny+1)
        index = indices[0, 0]
        ...
        t_last_y2[index], t_alpha_state_x, t_beta_state_x, t_alpha_state_y[index], t_beta_state_y[index], eps_x, eps_y[index], c_last_x1,
        ...
        row_prob = lax.cond(index % 2, lambda x: jnp.flip(x, 0), lambda x: x, row_prob)
        row_phase = lax.cond(index % 2, lambda x: jnp.flip(x, 0), lambda x: x, row_phase)
        #jax.debug.print("row_prob:{}", row_prob)
        #jax.debug.print("eps:{}", eps_x)
        return (input_x, input_y, t_last_x1, t_last_x2, t_last_y1s, t_last_y1e, t_last_y2, t_alpha_state_x1, t_beta_state_x1,
                t_alpha_state_y, t_beta_state_y, eps_x, eps_y, c_last_x1, c_last_x2, c_last_y1s, c_last_y1e, c_last_y2), (row_prob, row_phase)
    ...
    RWKV_net_params = params[19:]
    __, (probs, phase) = scan(scan_fun_2d, init, ny_nx_indices)
    #jax.debug.print("num_NaN:{}", jnp.sum(jnp.isnan(probs)))
    #jax.debug.print("num_inf:{}", jnp.sum(jnp.isinf(probs)))
    #jax.debug.print("num_inf:{}", jnp.sum(jnp.isneginf(probs)))
    log_probs, phase = jnp.sum(jnp.log(probs+1e-27)), jnp.sum(phase)
    log_amp = log_probs / 2 + phase * 1j/(Ny*Nx*py*px)
    
    return log_amp, probs
