...
parser.add_argument('--numsamples', type = int, default=4)
...
for angle in (0.10*jnp.pi, ):
    ...
    (xy_loc_bulk, xy_loc_edge, xy_loc_corner, yloc_bulk, yloc_edge, yloc_corner, zloc_bulk, zloc_edge,
     ...
     zloc_corner_diag, coe_bulk_diag, coe_edge_diag, coe_corner_diag) = vmc_off_diag_es(Ny, Nx, px, py, angle,
                                                                                        basis_rotation)
    T = T0
    t = time.time()
    ...
    for it in range(0, numsteps):
        ...
        jnp.save("debug_samples.npy", samples)
        ...
        if (it+1)%25==0 or it==0:
            ...
            print("diag_E = ", jnp.mean(batch_diag_coe(samples, zloc_bulk_diag, zloc_edge_diag, zloc_corner_diag, coe_bulk_diag, coe_edge_diag, coe_corner_diag)))
            if T0 != 0:
    ...
    print(time.time()-t)
    jnp.save("result/rwkv_meanE_L"+str(L)+"_units"+str(units)+"_cmi_pattern_"+cmi_pattern+"rotation"+str(basis_rotation)+"angle"+str(angle)+"_seed"+str(args.seed)+".npy", jnp.array(meanEnergy))
    jnp.save("result/rwkv_varE_L"+str(L)+"_units"+str(units)+"_cmi_pattern_"+cmi_pattern+"rotation"+str(basis_rotation)+"angle"+str(angle)+"_seed"+str(args.seed)+".npy", jnp.array(varEnergy))
