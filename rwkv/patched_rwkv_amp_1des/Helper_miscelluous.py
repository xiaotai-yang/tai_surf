...
def compute_cost(parameters, wemb, fixed_parameters, samples, Eloc, Temperature, ny_nx_indices, M0_, M_, Mlast_):
    samples = jax.lax.stop_gradient(samples)
    Eloc = jax.lax.stop_gradient(Eloc)

    # First term

    log_amps_tensor = (jax.jit(vmap(log_amp_RWKV, (0, None, None, None, None)), static_argnames=['fixed_params'])(samples, parameters, wemb, fixed_parameters, ny_nx_indices) +
