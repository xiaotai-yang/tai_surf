...
def compute_cost(parameters, fixed_parameters, wemb, samples, Eloc, Temperature, ny_nx_indices):
    samples = jax.lax.stop_gradient(samples)
    Eloc = jax.lax.stop_gradient(Eloc)

    # First term

    log_amps_tensor = vmap(log_amp_RWKV, (0, None, None, None, None))(samples, parameters, wemb, fixed_parameters, ny_nx_indices)
    ...
    return cost
def params_init(rnn_type, Nx, Ny, units, input_size, emb_size, h_size, num_layer, key):
    if (rnn_type == "vanilla"):
        ...
        out_size = input_size
        params = init_RWKV_params(input_size, emb_size, h_size, num_layer, out_size, Ny, Nx, key)
    return params
