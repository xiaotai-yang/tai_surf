...
def compute_cost(parameters, fixed_parameters, samples, Eloc, Temperature, n_indices, M0_, M_, Mlast_):
    samples = jax.lax.stop_gradient(samples)
    Eloc = jax.lax.stop_gradient(Eloc)

    # First term

    log_amps_tensor = (jax.jit(vmap(log_amp,(0, None, None, None)), static_argnames=['fixed_params'])(samples, parameters, fixed_parameters, n_indices)+
