...
ang_array = [0.0, 0.157, 0.314, 0.471, 0.628, 0.785, 0.942, 1.1, 1.257, 1.414, 1.571]
for L, p  in  zip([16, 32, 64], [1, 2, 1]):
    N = L
    input_size = 2**p
    a = 0
    ...
    for angle in (0.0*jnp.pi, 0.05*jnp.pi, 0.1*jnp.pi, 0.15*jnp.pi, 0.20*jnp.pi, 0.25*jnp.pi, 0.3*jnp.pi, 0.35*jnp.pi, 0.4*jnp.pi, 0.45*jnp.pi, 0.5*jnp.pi):
        ...
        for it in range(0, numsteps):
            ...
            log_all_amp = batch_log_amp(sigmas, params, fixed_params, n_indices) + batch_log_phase_dmrg(sigmas.reshape(-1, L*p), M0, M, Mlast)
            ...
            grads = grad_f(params, fixed_params, samples_grad, Eloc, T, n_indices, M0, M, Mlast)
